...
#[command(about = "Generate compact JSON schema refs and a SQLite path index from JSON/JSONL input")]
struct Args {
...
fn normalize_input(input: &str, jsonl_mode: bool, file_mode: bool, base: &str) -> Result<Vec<Entry>> {
    if input.trim().is_empty() {
        bail!("input is empty");
    }

    if jsonl_mode {
        let root = if file_mode { format!("{base}_ref") } else { "root_item".to_string() };
    ...
    let root_object = if file_mode { base.to_string() } else { "root".to_string() };
    let root_collection = if file_mode { format!("{base}_ref") } else { "root_item".to_string() };
...
fn distinct_schemas(outdir: &Path, entries: Vec<Entry>) -> Result<Vec<Record>> {
    ...
    for entry in entries {
        let role = if entry.collection { Role::RootCollection } else { Role::Object };
        collect_object_occurrences(entry.path, entry.value, role, entry.index, &mut occurrences);
    }

    let mut records = Vec::new();
    records.extend(object_records(
        outdir,
        occurrences.iter().filter(|o| o.role == Role::Object).cloned().collect(),
    )?);
    records.extend(array_records(
        outdir,
        occurrences.iter().filter(|o| o.role == Role::ArrayItem).cloned().collect(),
    )?);
    ...
    for (key, child) in value {
        match child {
            Value::Object(map) => {
                ...
                collect_object_occurrences(child_segments, map, Role::Object, None, out);
            }
            Value::Array(items) => {
                for (idx, item) in items.into_iter().enumerate() {
                    if let Value::Object(map) = item {
                        ...
                        collect_object_occurrences(child_segments, map, Role::ArrayItem, Some(idx), out);
                    }
                }
            }
            _ => {}
...
fn object_records(outdir: &Path, occurrences: Vec<Occurrence>) -> Result<Vec<Record>> {
    ...
    for (segments, objects) in by_segments {
        ...
        by_canonical.entry(canonical).or_default().push((schema_path, schema));
    }
...
fn array_records(outdir: &Path, occurrences: Vec<Occurrence>) -> Result<Vec<Record>> {
    ...
    for occ in occurrences {
        by_segments.entry(occ.segments.clone()).or_default().push(occ);
    }
    ...
    for (segments, items) in by_segments {
        ...
        for item in items {
            let index = item.array_index.context("array item missing index")?;
            let schema = schema_for_values(outdir, &segments, &[item.value])?;
            groups.entry(canonical_json(&schema)?).or_default().push((index, schema));
        }
        ...
        if groups.len() == 1 {
            ...
            let mut distinct = groups
                .into_values()
                .map(|same| {
                    let first_index = same.iter().map(|(i, _)| *i).min().unwrap();
                    (first_index, same)
                })
                .collect::<Vec<_>>();
            distinct.sort_by_key(|(first_index, _)| *first_index);

            for (collision_index, (first_index, same)) in distinct.into_iter().enumerate() {
                let file_stem = if collision_index == 0 {
                    first_index.to_string()
                } else {
                    format!("{first_index}_{collision_index}")
                };
                let mut array_indexes = same.iter().map(|(i, _)| *i).collect::<Vec<_>>();
...
fn root_collection_records(outdir: &Path, occurrences: Vec<Occurrence>) -> Result<Vec<Record>> {
    ...
    for (segments, items) in by_segments {
        ...
        let mut groups: BTreeMap<String, Vec<(usize, Value)>> = BTreeMap::new();
        for item in items {
            let index = item.array_index.context("root collection item missing index")?;
            let schema = schema_for_values(outdir, &segments, &[item.value])?;
            schemas.push(index);
            groups.entry(canonical_json(&schema)?).or_default().push((index, schema));
        }
        ...
        if groups.len() == 1 {
            ...
            for (first_index, same) in distinct {
                let mut array_indexes = same.iter().map(|(i, _)| *i).collect::<Vec<_>>();
                array_indexes.sort_unstable();
                array_indexes.dedup();
                records.push(Record {
                    schema_path: format!("{collection_base}/{first_index}.json"),
                    object_paths: vec![format!("{collection_base}/{first_index}.json")],
                    schema: same[0].1.clone(),
                    array_parent: Some(format!("{collection_base}.json")),
                    array_indexes,
                });
...
fn schema_for_values(outdir: &Path, segments: &[String], objects: &[Map<String, Value>]) -> Result<Value> {
    ...
    for key in keys {
        ...
        child_segments.push(key.clone());
        let path_text = child_segments.join("/");

        let label = if types.contains(&"array") {
            let member_types = unique_types(values.iter().filter_map(|v| v.as_array()).flatten());
            if member_types.contains(&"object") {
                if member_types.iter().any(|t| *t != "object") {
                    bail!("mixed object/non-object array at {path_text}");
                }
                format!("{}.json", reference_path(outdir, &child_segments))
            } else if member_types.contains(&"array") {
                bail!("nested array value at {path_text}");
            } else if types.iter().all(|t| matches!(*t, "array" | "null")) {
                primitive_array_label(&values)
            } else {
                types
                    .iter()
                    .filter(|t| **t != "null")
...
fn primitive_array_label(arrays: &[&Value]) -> String {
    let mut members = Vec::new();
    for value in arrays {
        if let Some(items) = value.as_array() {
            members.extend(items.iter());
        }
    }
    ...
    if members.is_empty() {
        ...
        format!("array({})", types[0])
    } else if types.len() == 2 && types[0] == "null" {
        format!("array({}?)", types[1])
    } else {
...
fn reference_path(outdir: &Path, segments: &[String]) -> String {
    let joined = segments.iter().map(|s| path_segment(s)).collect::<Vec<_>>().join("/");
...
fn write_files_and_index(outdir: &Path, records: &[Record]) -> Result<()> {
    ...
    for record in records {
        ...
        if let Some(parent) = &record.array_parent {
            for idx in &record.array_indexes {
                tx.execute(
                    "INSERT INTO array_index_refs(array_path, array_index, schema_path) VALUES (?1, ?2, ?3) \
                     ON CONFLICT(array_path, array_index) DO UPDATE SET schema_path=excluded.schema_path",
                    params![parent, *idx as i64, record.schema_path],
                )?;
            }
        }
    }
...
fn replace_with_symlink(target: &Path, link: &Path) -> Result<()> {
    ...
    #[cfg(unix)]
    {
        std::os::unix::fs::symlink(&relative, link)
            .with_context(|| format!("failed to symlink {} -> {}", link.display(), relative.display()))?;
    }
    #[cfg(not(unix))]
    {
...
mod tests {
    ...
    fn nullable_containers_use_schema_references_and_dump_nested_schemas() {
        ...
        assert_eq!(schema["items"], "refs/root/items.json");
